...
export function PaymentsTabs() {
  ...
  const loadTransactions = async () => {
    try {
      ...
      const escrow = (milestones || []).map((m: any) => ({
        id: m.id,
        title: m.title,
        amount: m.amount,
      ...
      const { data: transactions } = await supabase
        .from("transactions")
        .select("id, transaction_type, amount, created_at, status, withdrawal_method")
        .eq("user_id", user.id)
      ...
      const payments = (transactions || []).map((t: any) => ({
        ...
        title: t.transaction_type === "withdrawal" ? "Withdrawal" : "Payment received",
        amount: t.amount,
        date: new Date(t.created_at).toLocaleDateString(),
        status: t.status,
        method: t.withdrawal_method || "Direct Transfer",
      }))
                ...
                <Card key={payment.id} className="p-6 hover:border-secondary/50 transition-all">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      >
                        <Icon className={`w-5 h-5 ${isReceived ? "text-accent" : "text-primary"}`} />
                      </div>
                      <div>
